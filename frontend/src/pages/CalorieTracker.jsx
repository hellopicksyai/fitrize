...
export default function CalorieTracker() {
  ...
  const setWater = async (g) => {
    const glasses = Math.max(0, g);            // never below 0
    setToday((prev) => ({ ...prev, water_glasses: glasses })); // optimistic, instant UI
    try {
  ...
  const addWater = () => setWater((today.water_glasses || 0) + 1);
  const removeWater = () => setWater((today.water_glasses || 0) - 1);
      ...
      <Card className="p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 glass glow-hover" data-testid="water-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="display text-2xl">{today.water_glasses || 0} / 8 glasses</div>
            {/* visual glass dots */}
            <div className="flex gap-1.5 mt-2">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i}
                  className={`w-3.5 h-5 rounded-b-md rounded-t-sm border transition-all ${
                    i < (today.water_glasses || 0)
        ...
        <div className="flex items-center gap-2">
          <Button onClick={removeWater} variant="outline" size="icon" className="rounded-full"
            disabled={(today.water_glasses || 0) <= 0} data-testid="btn-water-remove" aria-label="Remove a glass">
            <Minus className="w-4 h-4"/>
          </Button>
          <Button onClick={addWater} className="rounded-full" data-testid="btn-water-add"><Plus className="w-4 h-4 mr-1"/>Glass</Button>
        </div>
      </Card>
